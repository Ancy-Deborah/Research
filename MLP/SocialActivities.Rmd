...
stud<-stud[c(18:30,34)]
as.factor(stud)
names(stud)

#0CONVERTIG TO NUMERIC
stud$paid<-as.numeric(stud$paid)
stud$activities<-as.numeric(stud$activities)
stud$higher<-as.numeric(stud$higher)
stud$internet<-as.numeric(stud$internet)
stud$nursery<-as.numeric(stud$nursery)
stud$romantic<-as.numeric(stud$romantic)
...
StudentValues <- stud[,1:13]
StudentTargets <- decodeClassLabels(stud[,14])
...
model <- mlp(stud$inputsTrain, stud$targetsTrain, size=10, learnFuncParams=c(0.1), 
